...
def connect_to__db() -> str:
    ...
    """
    try:
        conn = sqlite3.connect('/home/kronos/Desktop/raman.db')
        conn.close()
...
async def process_patient_data(
    ...
    try:
        ...
        if li:
            ...
            summary_agent.run(f"{main_agent.memory.steps[-1].action_output} is the possible side affects for the drugs that the patient has been taking and it has been  synthsized from FDA data within the drugs mentioned \
                  in {li}, use the {li} to mention the drug names and give a \
                  aprisal to the paitent by advocating the risks involved in a manner that \
                  is easier to understand, summarizing the interactions of drugs and not frightening and also give him questions he can take back to the doctor and this entire thing has to be my final answer and you will start this like a human speaking""")
